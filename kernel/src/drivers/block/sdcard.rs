...
impl</* 'a, */ X: SPI> SDCard</* 'a, */ X> {
    ...
    pub fn init(&self) -> Result<SDCardInfo, InitError> {
        ...
        /* SD chip select high */
        self.CS_HIGH();
        /* NOTE: this reset doesn't always seem to work if the SD access was broken
         * off in the middle of an operation: CMDFailed(CMD0, 127). */
...
/** CS value passed to SPI controller, this is a dummy value as SPI0_CS3 is
 * not mapping to anything in the FPIOA */
const SD_CS: u32 = 3;
...
fn io_init() {
    ...
    fpioa::set_io_pull(io::SPI0_CS0, fpioa::pull::DOWN); // GPIO output=pull
                                                         // down
}
...
pub struct SDCardWrapper(SDCard<SPIImpl<SPI0>>);

unsafe impl Send for SDCardWrapper {}
unsafe impl Sync for SDCardWrapper {}
